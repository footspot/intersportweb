...
const reasonOptions = computed(() => [
  t('admin.fund.debit.reasons.jerseys'),
  t('admin.fund.debit.reasons.training'),
  t('admin.fund.debit.reasons.accessories'),
  t('admin.fund.debit.reasons.shoes'),
  t('admin.fund.debit.reasons.flocking'),
  t('admin.fund.debit.reasons.other'),
])
...
const wouldGoNegative = computed(() => nextBalance.value < 0)
...
async function save() {
  ...
  saving.value = true
  try {
    await fund.debit(props.club.id, a, reason.value.trim(), reference.value.trim() || undefined)
    emit('saved')
  ...
  >
    <div class="w-full max-w-md bg-white dark:bg-sidebar-surface rounded-card shadow-card-lg p-6 space-y-4">
      <div class="flex items-center gap-3">
        <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-brand-gold/10 text-brand-gold">
          <UIcon name="i-lucide-minus-circle" class="w-5 h-5" />
        </span>
        <div>
          <h3 class="font-heading text-lg font-bold">{{ t('admin.fund.debit.title') }}</h3>
      ...
      <label class="block">
        ...
        <div class="flex items-center gap-2 mt-1">
          <input
            ...
            min="0"
            step="0.01"
            class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-sidebar bg-transparent focus:ring-2 focus:ring-brand-gold focus:outline-none"
          />
      ...
      <label class="block">
        ...
        <select
          v-model="reason"
          class="mt-1 w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-sidebar bg-white dark:bg-sidebar-surface focus:ring-2 focus:ring-brand-gold focus:outline-none"
        >
      ...
      <label class="block">
        ...
        <input
          ...
          class="mt-1 w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-sidebar bg-transparent focus:ring-2 focus:ring-brand-gold focus:outline-none"
        />
      </label>
      ...
      <div class="grid grid-cols-3 items-center gap-3 p-3 rounded-lg bg-gray-50 dark:bg-sidebar text-sm">
        ...
        <div class="text-center text-brand-gold">
        ...
        <div class="text-right">
          ...
          <div class="font-heading font-bold" :class="wouldGoNegative ? 'text-brand-secondary' : ''">
            {{ fmt(nextBalance) }}
          </div>
        </div>
      </div>

      <p v-if="wouldGoNegative" class="text-xs text-brand-secondary">
        <UIcon name="i-lucide-alert-triangle" class="w-3 h-3 inline mr-1" />
        {{ t('admin.fund.warningNegative') }}
      </p>
      ...
      <div class="flex justify-end gap-2 pt-2">
        ...
        <button
          type="button"
          class="px-4 py-2 rounded-lg text-sm font-medium bg-brand-gold text-white hover:opacity-90 disabled:opacity-60"
          :disabled="saving"
          @click="save"
        >
          {{ saving ? t('common.loading') : t('admin.fund.debit.confirm') }}
        </button>
      </div>
    </div>
  </div>
