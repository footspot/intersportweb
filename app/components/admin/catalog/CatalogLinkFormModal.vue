...
async function save() {
  errorMsg.value = null
  if (!name.value.trim()) {
    errorMsg.value = t('admin.catalog.errors.nameRequired')
    return
  }
  if (!url.value.trim()) {
    errorMsg.value = t('admin.catalog.errors.urlRequired')
    return
  }
  saving.value = true
  try {
    const clearLogo = !logoFile.value && !logoPath.value && !!props.link?.logo_path
    const payload = {
      name: name.value.trim(),
      url: url.value.trim(),
      sort_order: props.link?.sort_order ?? catalog.items.length,
      clear_logo: clearLogo,
      file: logoFile.value,
    }
    if (props.link) {
      await catalog.update({ id: props.link.id, ...payload })
    } else {
  ...
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
    <div class="w-full max-w-lg bg-white dark:bg-sidebar-surface rounded-card shadow-card-lg p-6 space-y-4">
      <h3 class="font-heading text-xl font-bold">
        {{ link ? t('admin.catalog.edit') : t('admin.catalog.new') }}
      </h3>

      <label class="block">
        <span class="text-sm font-medium">{{ t('admin.catalog.name') }}</span>
      ...
      <label class="block">
        <span class="text-sm font-medium">{{ t('admin.catalog.url') }}</span>
        <input v-model="url" type="url" placeholder="https://…"
          class="mt-1 w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-sidebar bg-transparent focus:ring-2 focus:ring-brand-primary focus:outline-none" />
      </label>

      <AdminImageUploader v-model:path="logoPath" v-model:file="logoFile" bucket="catalog-logos"
        :label="t('admin.catalog.logo')" />

      <p v-if="errorMsg" class="text-sm text-brand-secondary">{{ errorMsg }}</p>

      <div class="flex justify-end gap-2 pt-2">
        <button type="button" class="px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 dark:hover:bg-sidebar"
